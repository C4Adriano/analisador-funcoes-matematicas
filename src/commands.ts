import { Config, isConfigKey, type ConfigKey, type ConfigType } from "./config.js"
...
type CommandAction = (arg: boolean | null, parts: [cmd: string, argRaw: string]) => CommandsNames | null
...
const COMMANDS_NAMES: CommandsNames[] = ["config", "exit", "start", "review", "history", "change"]
...
function change<K extends ConfigKey>(name: K, value?: ConfigType[K] | null): null {
    const currentValue = Config[name]

    if (value == null && typeof currentValue === "boolean") Object.assign(Config, { [name]: !currentValue })
...
function help(specific: string): null {
    const cmds = listCmds()
...
function levenshtein(source: string, target: string): number {
    ...
    const rows = target.length + 1,
        ...
        matrix: NumericMatrix = Array.from({ length: rows }, (_, row) => Array.from({ length: cols }, (__, col) => (row === 0 ? col : col === 0 ? row : 0)))
...
function listCmds(): CommandRegistry {
    return {
        ...
        config: {
            ...
            action: (arg, [, argRaw]): "config" | null => {
                if (argRaw.trim() === "") {
                    State.type = "config"
                    return "config"
                }

                if (isConfigKey(argRaw)) return change(argRaw, arg)
        ...
        textcase: {
            ...
            action: (_, [, argRaw]): null => {
                const TEXT_CASES: TextCase[] = ["capitalized", "uppercase", "lowercase", "default"],
                    hasArg = argRaw.trim() !== "",
                    target = hasArg ? noAccents(lowercase(argRaw)) : "",
                    TEXT_CASE_ALIASES: Record<TextCase, string[]> = {
                ...
                if (hasArg) {
                    const match = (Object.entries(TEXT_CASE_ALIASES) as [TextCase, string[]][]).find(([, aliases]) => aliases.includes(target))?.[0]
                    ...
                    value = match
                } else value = TEXT_CASES[(TEXT_CASES.indexOf(Config.textCase) + 1) % TEXT_CASES.length] ?? "default"
        ...
        degrees: {
            ...
            action: (_, [, argRaw]): null => {
                const hasArg = argRaw.trim() !== "",
                    target = hasArg ? noAccents(lowercase(argRaw)) : "",
                    DEGREES_ALIASES: Record<Degrees, string[]> = { deg: ["graus", "grau", "degrees", "degree", "deg"], rad: ["radianos", "radians", "rad"] }
                let value: Degrees

                if (hasArg) {
                    const match = (Object.entries(DEGREES_ALIASES) as [Degrees, string[]][]).find(([, aliases]) => aliases.includes(target))?.[0]
                    ...
                    value = match
                } else value = Config.degrees === "deg" ? "rad" : "deg"
        ...
        language: {
            ...
            action: (_, [cmd, argRaw]): null => {
                const LANGUAGE_ALIASES: Record<Language, string[]> = {
                    ...
                    hasArg = argRaw.trim() !== "",
                    target = noAccents(lowercase(hasArg ? argRaw : cmd)),
                    entries = Object.entries(LANGUAGE_ALIASES) as [Language, string[]][],
                    match = entries.find(([, aliases]) => aliases.includes(target))?.[0]

                if (match != null) {
                    if (changeLanguage(match) === "same") notify(tr("commands.languageAlready"), { type: "warning" })
                    return null
                }

                if (hasArg) notify(tr("commands.invalidLanguage"), { explanation: `"${target}" ${tr("commands.noteInvalidLanguage")}`, type: "error" })
...
function processCommand(raw: string): CommandsNames | null {
    let input = raw

    for (;;) {
        if (input.length === 0 || !input.startsWith("/")) return null

        const [cmd = "", argRaw = ""] = noAccents(input.slice(1).toLowerCase()).split(" ", 2),
            arg = parseBool(argRaw),
            canonical = resolveCmd(cmd)
        ...
        if (canonical == null) {
            ...
            if (suggestion.type === "suggestion") {
                if (
                    notify(tr("commands.commandSuggestion", { suggestion: suggestion.canonical }), {
...
function resolveCmd(specific: string): string | null {
    if (specific.trim() === "") return null
    const cmds = listCmds(),
...
function searchCmds(term: string): string[] {
    if (term.trim() === "") return []

    const cmds = listCmds(),
        normalizedTerm = noAccents(lowercase(term))
...
function searchHelp(term: string): null {
    if (term.trim() === "") {
...
function shortcuts(specific: string): null {
    if (specific.trim() === "") {
    ...
    const variations = listCmds()
        [canonical]!.variations.map(v => `/${v}`)
        .join("\n")
    notify(`${tr("commands.commandVariations")}“/${canonical}”:\n${variations}`)
    return null
}

function suggestCmd(typed: string): { type: "exact" | "suggestion" | "unknown"; canonical: string; distance: number } {
    const cmds = listCmds()
    let best = "",
        lowerDist = Infinity
    ...
    for (const [key, cmd] of Object.entries(cmds))
        for (const candidate of [key, ...cmd.variations]) {
            ...
            if (dist >= lowerDist) continue

            lowerDist = dist
            best = key
        }

    if (lowerDist <= 3) return { type: lowerDist === 0 ? "exact" : "suggestion", canonical: best, distance: lowerDist }
...
function isValidCommand(value: unknown): value is CommandsNames {
    return typeof value === "string" && (COMMANDS_NAMES as readonly string[]).includes(value)
}
