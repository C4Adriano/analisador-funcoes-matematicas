...
type CommandMap = Record<Text, CommandItem>

type CommandSuggestionResult =
    | { type: "exact" | "suggestion"; canonical: Text; distance: number }
...
export const Commands = {
     ...
     * @since v6.1.0
     */
    process(raw: Text = ""): Text | null {
        if (typeof raw !== "string" || raw.length === 0 || raw[0] !== "/") {
            return null
        }

        const parts: Text[] = Writing.noAccents(raw.slice(1).toLowerCase()).split(" ")
        const cmd: Text = parts[0] ?? ""
        const arg: boolean | undefined = Commands.parseBool(parts[1] ?? "")
        const canonical: Text | null = Commands.resolveCmd(cmd)
        const cmds: CommandMap = Commands.listCmd()
     ...
     * @since v6.1.0
     */
    levenshtein(wrong: Text = "", correct: Text = ""): Numeric {
     ...
     * @since v6.1.0
     */
    suggestCmd(typed: Text = ""): CommandSuggestionResult {
        const LIMIT = 3
        const cmds: CommandMap = Commands.listCmd()
        const keys: Text[] = Object.keys(cmds)
        let best: Text = "",
            lowerDist: Numeric = Infinity,
            candidates: Text[] = []
    ...
    listCmd(): CommandMap {
        return {
            help: {
                ...
                action(arg: boolean | undefined, parts: Text[]) {
     ...
     * @since v6.1.0
     */
    resolveCmd(specific: Text = ""): Text | null {
        if (typeof specific !== "string" || specific === "") {
            return null
        }

        const cmds: CommandMap = Commands.listCmd()
        const cmdKeys: Text[] = Object.keys(cmds)
        let canonical: Text = specific
     ...
     * @since v6.1.0
     */
    parseBool(text: Text = ""): boolean | undefined {
    ...
    help(specific: Text = ""): null {
        ...
        if (specific != "") {
            const canonical: Text | null = Commands.resolveCmd(specific)
        ...
        const key: Text[] = Object.keys(cmds)
        const total: number = Math.ceil(key.length / 5)
        let page: number = 1,
            answer: number = 0
        ...
        do {
            ...
            const end: number = Math.min(start + 5, key.length)
            let menu: Text =
                "=== " +
                tr("Ajuda", "Help") +
                " ===" +
            ...
            answer = Ui.range(menu, "", 0, 9, 0, true) as Numeric
    ...
    searchHelp(term: Text = ""): null {
        ...
        const results: Text[] = Commands.searchCmd(term)
        const cmds: CommandMap = Commands.listCmd()
        ...
        const total: number = Math.ceil(results.length / 5)
        let page: number = 1,
            answer: number = 0
        ...
        do {
            ...
            const start: number = (page - 1) * 5
            const end: number = Math.min(start + 5, results.length)
            let menu: Text =
                "=== " +
                tr("Pesquisa", "Search") +
                ": “" +
    ...
    shortcuts(specific: Text = ""): null {
        ...
        const all: Text[] = cmds[canonical]!.variations
        const list: Text = all
            .map(function (v) {
                return "“/" + v
            })
    ...
    change<K extends ConfigKey>(name: K, value?: (typeof Config)[K]): null {
        ...
        if (value !== undefined) {
            ...
            Config[name] = !currentValue as (typeof Config)[K]
        } else {
