...
const COMMANDS_NAMES = ["config", "exit", "start", "review", "history", "change"],
    Commands = {
        process: (raw = "") => {
            ...
            return Commands.listCmds[canonical]?.action(arg, parts) ?? null
        },
        ...
        searchCmds: (term = "") => {
            ...
            return Object.keys(cmds).filter(key => {
                ...
                return (
                    cmd != null &&
                    [key, cmd.short, cmd.long, ...cmd.variations].some(candidate =>
        ...
        get listCmds() {
            return {
                ...
                textcase: {
                    ...
                    action: (_, parts) => {
                        /** @type {TextCase[]} */ const TEXT_CASES = [
                                "capitalized",
                                "uppercase",
                                "lowercase",
                                "normal",
                        ...
                        if (target == null)
                            value =
                                TEXT_CASES[(TEXT_CASES.indexOf(Config.textCase) + 1) % TEXT_CASES.length] ?? "normal"
                        else {
        ...
        searchHelp: (term = "") => {
            ...
            do {
                ...
                menu += results
                    .slice(start, end)
                    .map(name => `\n/${name} — ${Commands.listCmds[name].short}`)
                    .join("")
        ...
        shortcuts: (specific = "") => {
            ...
            const variations = Commands.listCmds[canonical].variations.map(v => `/${v}`).join("\n")
        ...
        /**
         * Altera um valor em `Config`.
         * @template {config.ConfigKey} K - Chave.
         * @param {K} name - Nome.
         * @param {config.ConfigType[K]} value - Valor.
         */
        change: (name, value) => {
...
export { Commands, COMMANDS_NAMES }
