...
const BASE_OPTIONS = ["analyze.options.domain", "analyze.options.range", "analyze.options.xIntersection", "analyze.options.yIntersection", "analyze.options.xValues", "analyze.options.yValues", "analyze.options.signAnalysis", "analyze.options.functionEquations"];
function resolveConstant({ c = State.current.numericC } = {}) {
    const coefs = { a: 0, b: 0, c }, showC = decimalOptions(coefs.c);
...
function resolveAffine({ b = State.current.numericB, c = State.current.numericC } = {}) {
    const coefs = { a: 0, b, c };
    resolveFunction(coefs);
...
function resolveQuadratic({ a = State.current.numericA, b = State.current.numericB, c = State.current.numericC } = {}) {
    const coefs = { a, b, c };
    resolveFunction(coefs);
    const [delta, root1, root2] = calculateDelta(coefs), [x1, x2] = [decimalOptions(root1), decimalOptions(root2)], [vertexX, vertexY] = vertex(coefs.a, coefs.b, delta), [p1, p2] = [decimalOptions(vertexX), decimalOptions(vertexY)];
...
function resolveExponential({ a = State.current.numericA, b = State.current.numericB, c = State.current.numericC } = {}) {
    const coefs = { a, b, c };
    resolveFunction(coefs, "exp");
    const root = calculateRoot(coefs, "exp"), showC = decimalOptions(coefs.c);
    runAnalysisMenu(coefs, "exp", ["analyze.options.curve", "analyze.options.root", "analyze.options.horizontalAsymptote"], {
        1: {
            ...
            5: () => {
                if (coefs.b > 0)
                    range(`∈ (${showC}, ∞)`, tr("analyze.betweenCInfinity"));
                else
                    range(`∈ (-∞, ${showC})`, tr("analyze.betweenInfinityC"));
            },
        },
...
function resolveLogarithmic({ a = State.current.numericA, b = State.current.numericB, c = State.current.numericC } = {}) {
    const coefs = { a, b, c };
...
function resolveSine({ a = State.current.numericA, b = State.current.numericB, c = State.current.numericC } = {}) {
    const coefs = { a, b, c };
...
function resolveCosine({ a = State.current.numericA, b = State.current.numericB, c = State.current.numericC } = {}) {
    const coefs = { a, b, c };
...
function resolveTangent({ a = State.current.numericA, b = State.current.numericB, c = State.current.numericC } = {}) {
    const coefs = { a, b, c };
...
function runAnalysisMenu(coefs, funcType, extraOptions, pageActions) {
    let option, [page, limit] = [1, 0];
    do {
        ;
        [option, page] = menu(trArr([...extraOptions, ...BASE_OPTIONS]), page);
        if (isValidCommand(option))
            [option, page] = [0, 1];
        if (isFiniteNumber(option)) {
            const result = pageActions[page]?.[option]?.();
            if (isValidText(result))
                option = result === "result" ? 1 : 0;
        }
        if (option === 6)
            resolveFunction(coefs, funcType, true);
        limit++;
