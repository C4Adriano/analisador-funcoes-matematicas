...
loadConfig();
// Introdução
Ui.display(tr("Bem-vindo ao Analisador de Funções Matemáticas!", "Welcome to the Mathematical Function Analyzer!"), tr("Este programa analisa funções dos tipos constante, afim, quadrática, exponencial, logarítmica, seno, cosseno, tangente, etc. — identificando suas propriedades e características. Para começar, informe os dados da função quando solicitado.", "This program analyzes functions of the types constant, affine, quadratic, exponential, logarithmic, sine, cosine, tangent, etc. — identifying their properties and characteristics. To get started, enter the function data when prompted."));
// === OBJETOS GLOBAIS ===
...
const description = tr("Analisador de Funções Matemáticas desenvolvido em JavaScript e TypeScript.", "Mathematical Function Analyzer developed in JavaScript and TypeScript.");
const locales = {
    pt: "pt_BR",
    en: "en_US",
};
...
let subType = 0, subLoop = false, choice = 0, page = 1;
...
do {
    ...
    // Salva histórico
    if (State.globalA != State.currentFunc[0] ||
    ...
    if ((typeof State.type === "number" &&
        ...
        if (State.type == 1) {
            // Incógnitas
            if (!Checks.isNumeric(State.globalA) ||
        ...
        // Não polinomial
            ...
            do {
                subType = Ui.input("=== Menu ===\n" +
                    ...
                    tr("Alterar", "Change") +
                    " | 0 = " +
                    tr("Voltar", "Back"), "", true, 0, true);
                subLoop = false;
                if ((typeof subType === "number" && 0 <= subType && subType <= 2) ||
                    (typeof subType === "number" && 6 <= subType && subType <= 9) ||
                    (typeof subType === "string" && Commands.names().includes(subType))) {
                    // Exponencial
                    if (subType == 1) {
                        // Incógnitas
                        if (typeof State.globalA === "string" ||
        ...
        // Trigonométrica
            ...
            do {
                subType = Ui.input("=== Menu ===\n" +
                    ...
                    tr("Voltar", "Back"), "", true, 0, true);
                subLoop = false;
                if ((typeof subType === "number" && 0 <= subType && subType <= 3) ||
                    (typeof subType === "number" && 6 <= subType && subType <= 9) ||
                    (typeof subType === "string" && Commands.names().includes(subType))) {
                    // Seno
                    if (subType == 1) {
        ...
        // Histórico
            ...
            if (State.history.length <= 1) {
                Ui.display(tr("Não há histórico o suficiente para mudanças.", "There is no history yet for changes."), tr("Escrevestes apenas uma função até agora. Use “alterar” para escrever outra função.", "You have written only one function so far. Use “changes” to write another function."));
            }
            else {
                let message = "=== " +
                    tr("Histórico", "History") +
        ...
        // Configurações
            ...
            do {
                ...
                let total = Math.ceil(configOptions.length / 6), option = 1;
                // Controla página
                if (page < 1) {
                    page = 1;
                }
                ...
                if (choice == 7) {
                    ...
                    if (JSON.stringify(Config) == JSON.stringify(DEFAULT_CONFIG)) {
                        Ui.warning(tr("Todas as configurações já estão na forma padrão.", "All settings are already at their default values."), tr("Não há necessidade de restaurar.", "There is no need to restore."));
                    }
                    else {
                        let message = tr("Voltar às configurações padrão?\nConfigurações afetadas:\n", "Do you want to restore the default settings?\nAffected settings:\n"), keys = Object.keys(Config);
                        ...
                        // Confirmação
                        if (Ui.warning(message, tr("Obs.₁: Isso irá afetar todas as configurações acima\nObs.₂: Essa alteração é permanente", "Note₁: This will affect all the settings above\nNote₂: This change is permanent"), true)) {
                            resetConfig();
                        }
                    }
                }
                ...
                if (page == 1) {
                    // Unicode
                    if (choice == 1) {
                        Config.unicode = Ui.confirm(Writing.configItem(tr("Ativar caracteres Unicode?", "Enable Unicode characters?"), "unicode"), tr("Obs.₁: Caracteres Unicode são os símbolos especiais, tais como: “ℝ”, “∀”, etc. Desativar fará com que eles sejam transformados em uma palavra correspondente, tais como: “Reais”, “para todo”, etc.\nObs.₂: Nem todos os caracteres Unicode serão desativados\nObs.₃: Essa configuração pode mudar algumas explicações", "Note₁: Unicode characters are special symbols, such as: “ℝ”, “∀”, etc. Disabling will replace them with a corresponding word, such as: “Reals”, “for all”, etc.\nNote₂: Not all Unicode characters will be disabled\nNote₃: This setting may change some explanations"));
                    }
                    // Explicações
                    else if (choice == 2) {
                        Config.explanations = Ui.confirm(Writing.configItem(tr("Ativar explicações?", "Enable explanations?"), "explanations"), tr("Obs.₁: Ativar fará com que certas mensagens sejam diferentes e tenham explicações, por exemplo: o cálculo do Delta, Δ = b² - 4 · a · c, sem ser só o resultado dele\nObs.₂: Nem todas as mensagens têm versão explicada\nObs.₃: Desativar o Unicode fará com que seja mostrado: Delta = b^2 - 4 * a * c", "Note₁: Enabling will make certain messages different and include explanations, e.g.: the Delta calculation, Δ = b² - 4 · a · c, not just the result\nNote₂: Not all messages have an explained version\nNote₃: Disabling Unicode will show: Delta = b^2 - 4 * a * c"));
                    }
                    // Acentos
                    else if (choice == 3) {
                        Config.accents = Ui.confirm(Writing.configItem(tr("Ativar acentos?", "Enable accents?"), "accents"), tr("Obs.: Essa configuração irá tirar todos os acentos gráficos das palavras, podendo haver má interpretação", "Note: This setting will remove all accents from words, which may cause misinterpretation"));
                    }
                    // Capitalizadas
                    else if (choice == 4) {
                        Config.capitalized = Ui.confirm(Writing.configItem(tr("Ativar letras capitalizadas?", "Enable capitalized letters?"), "capitalized"), tr("Obs.₁: Essa configuração irá transformar as palavras em “normais”, no caso, a primeira letra da frase em maiúscula e as outras todas em minúsculas\nObs.₂: Essa configuração irá desativar “maiúsculas” e “minúsculas”", "Note₁: This setting will make text “normal”, i.e., the first letter of each sentence capitalized and the rest lowercase\nNote₂: This setting will disable “uppercase” and “lowercase”"));
                    ...
                    // Maiúsculas
                        ...
                        Config.uppercase = Ui.confirm(Writing.configItem(tr("Ativar todas as letras maiúsculas?", "Enable all uppercase letters?"), "uppercase"), tr("Obs.₁: Essa configuração irá transformar todas as letras em maiúsculas\nObs.₂: Essa configuração irá desativar “capitalizadas” e “minúsculas”", "Note₁: This setting will transform all letters to uppercase\nNote₂: This setting will disable “capitalized” and “lowercase”"));
                        if (Config.uppercase) {
                    ...
                    // Minúsculas
                        ...
                        Config.lowercase = Ui.confirm(Writing.configItem(tr("Ativar todas as letras minúsculas?", "Enable all lowercase letters?"), "lowercase"), tr("Obs.₁: Essa configuração irá transformar todas as letras em minúsculas\nObs.₂: Essa configuração irá desativar “capitalizadas” e “maiúsculas”", "Note₁: This setting will transform all letters to lowercase\nNote₂: This setting will disable “capitalized” and “uppercase”"));
                        if (Config.lowercase) {
                            Config.capitalized = false;
                            Config.uppercase = false;
                        }
                ...
                // Página 2
                    ...
                    // Ponto decimal
                    if (choice == 1) {
                        Config.decimalSeparator = Ui.confirm(Writing.configItem(tr("Alterar ponto decimal?", "Change decimal separator?"), "decimalSeparator"), tr("Obs.₁: Essa configuração irá transformar os números com “.” em números com “,”, por exemplo: ", "Note₁: This setting will transform numbers with “.” into numbers with “,”, e.g.: ") +
                    ...
                    // Multiplicação simples
                        ...
                        Config.simpleMulti = Ui.confirm(Writing.configItem(tr("Alterar para multiplicação simples?", "Enable simple multiplication?"), "simpleMulti"), tr("Obs.₁: Isso irá alterar esteticamente as contas polinomiais de: “a · x² + b · x + c” para: “ax² + bx + c”\nObs.₂: Desativar o Unicode irá transformar o “·” em *\nObs.₃: Isso não irá afetar o “×”, porém o Unicode irá transformá-lo em *", "Note₁: This will aesthetically change polynomial expressions from: “a · x² + b · x + c” to: “ax² + bx + c”\nNote₂: Disabling Unicode will transform “·” into *\nNote₃: This will not affect “×”, but Unicode will transform it into *"));
                    }
                    // Confirmações de entrada
                    else if (choice == 3) {
                        Config.inputConfirm = Ui.confirm(Writing.configItem(tr("Ativar confirmações de entrada?", "Enable input confirmations?"), "inputConfirm"), tr("Obs.: Toda e qualquer coisa digitada passará a ter que ser confirmada", "Note: Everything typed will need to be confirmed"));
                    }
                    // Confirmações de saída
                    else if (choice == 4) {
                        Config.outputConfirm = Ui.confirm(Writing.configItem(tr("Ativar confirmações de saída?", "Enable exit confirmations?"), "outputConfirm"), tr("Obs.: Isso irá ativar uma mensagem antes de sair / fechar o programa", "Note: This will show a confirmation message before exiting / closing the program"));
                    }
                    // Errors
                    else if (choice == 5) {
                        Config.errors = Ui.confirm(Writing.configItem(tr("Ativar mensagens de erro?", "Enable error messages?"), "errors"), tr("Obs.: Desativar pode fazer com que tu não percebas algum erro que estás cometendo", "Note: Disabling may cause you not to notice errors you are making"));
                    }
                    // Função
                    else if (choice == 6) {
                        Config.showFunction = Ui.confirm(Writing.configItem(tr("Ativar exibição da função?", "Enable function display?"), "showFunction"), tr("Obs.₁: “Mostrar função” significa que será mostrada a função (por exemplo: ax² + bx + c) no começo dos menus, antes das opções\nObs.₂: A função ainda continuará sendo mostrada quando for escolhida a opção “6” (Rever / Mostrar função)", "Note₁: “Show function” means the function (e.g.: ax² + bx + c) will be shown at the start of menus, before the options\nNote₂: The function will still be shown when option “6” (Review / Show function) is selected"));
                    }
                }
                // Página 3
                else if (page == 3) {
                    ...
                    // Precisão do log
                        ...
                        Config.logPrecision = Ui.range(Writing.configItem(tr("Qual a precisão do log?", "What is the log precision?"), "logPrecision"), tr("Obs.₁: Isso poderá afetar contas muito pequenas envolvendo logs\nObs.₂: Tu terás que escrever literalmente “1e-12”", "Note₁: This may affect very small calculations involving logarithms\nNote₂: You will have to literally type “1e-12”"), 1e-12, 1e-6, 20);
                    }
                    // Precisão da divisão
                    else if (choice == 3) {
                        Config.divPrecision = Ui.range(Writing.configItem(tr("Qual a precisão da divisão?", "What is the division precision?"), "divPrecision"), tr("Obs.₁: Isso poderá afetar contas muito pequenas envolvendo divisões\nObs.₂: Tu terás que escrever literalmente “1e-12”", "Note₁: This may affect very small calculations involving divisions\nNote₂: You will have to literally type “1e-12”"), 1e-12, 1e-6, 20);
                    }
                    // Limite de interações
                    else if (choice == 4) {
                        Config.interactionLimit = Ui.range(Writing.configItem(tr("Qual o limite de interações?", "What is the iteration limit?"), "interactionLimit"), tr("Obs.₁: Isso irá afetar todos os loops, tais como logs, menus, etc.\nObs.₂: Essa configuração é útil para evitar loops infinitos no código, caso algo dê errado", "Note₁: This will affect all loops, such as logarithms, menus, etc.\nNote₂: This setting is useful to avoid infinite loops in the code, in case something goes wrong"), 100, 10000);
                    }
                    // Linguagem
                    else if (choice == 5) {
                        let question = Ui.range(Writing.configItem(tr("Qual língua?", "Which language?"), "language") +
                            "\n1 = " +
                            tr("Português Brasileiro", "Brazilian Portuguese") +
                            "\n2 = " +
                            tr("Inglês", "English"), tr("Obs.: Isso irá alterar a língua do sistema inteiro.", "Note: This will change the entire system language."), 1, 2, 0), language = question == 1 ? "pt" : "en";
                    ...
                    // Graus
                        ...
                        const useDegrees = Ui.confirm(Writing.configItem(tr("Usar graus em vez de radianos?", "Use degrees instead of radians?"), "degrees"), tr("Obs.₁: Isso irá afetar as funções trigonométricas, tais como seno, cosseno e tangente\nObs.₂: Ativar isso irá fazer com que os ângulos sejam interpretados como graus, e não π radianos", "Note₁: This will affect trigonometric functions, such as sine, cosine and tangent\nNote₂: Enabling this will make angles be interpreted as degrees, not π radians"));
        ...
        // Sair
            ...
            if (Config.outputConfirm) {
                State.loop = !Ui.confirm(tr("Tu queres sair?", "Do you want to exit?"), tr("Obs.: As configurações poderão ser redefinidas ao sair.", "Note: Settings may be reset upon exit."));
            }
            else {
                State.loop = false;
            }
