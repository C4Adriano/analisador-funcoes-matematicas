...
export const teste = {
    ...
    contas() {
        ...
        for (let i = 0; i < resultados.length; i++) {
            let nome = resultados[i][0], obtido = resultados[i][1], esperado = resultados[i][2]
            let passou = (isNaN(esperado) ? isNaN(obtido) : obtido == esperado)
            if (!passou) { erros++ }
            relatorio += (passou ? "✓" : "✗") + " " + nome + " → " + obtido + (passou ? "" : " (esperado: " + esperado + ")") + "\n"
        }
